<div align="center">

# 你好，我是 Yanhuo

**全栈开发者 · Java / Vue / Python**

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&weight=500&size=20&pause=1200&color=38BDF8&center=true&vCenter=true&width=520&lines=Java+后端+%C2%B7+Vue+前端+%C2%B7+Python+自动化;关注工程实践与代码质量)](https://git.io/typing-svg)

</div>

## 关于我

主要使用 **Java / Spring Boot** 构建后端服务，使用 **Vue 3 / TypeScript** 完成前端开发，也会用 **Python** 处理自动化、数据分析与接口测试。

关注可维护性、工程化与稳定交付。

> Write clean code. Build meaningful things.

## 当前关注

- 构建个人全栈应用，持续打磨 Spring Boot 与 Vue 3 的协作方式
- 学习 Spring Cloud 微服务与 Java 性能调优
- 期待参与实用的开源工具和工程化项目

## 技术栈

**后端**

![Java](https://img.shields.io/badge/Java-ED8B00?style=flat-square&logo=openjdk&logoColor=white)
![Spring Boot](https://img.shields.io/badge/Spring_Boot-6DB33F?style=flat-square&logo=spring-boot&logoColor=white)
![MyBatis](https://img.shields.io/badge/MyBatis-111111?style=flat-square&logo=apache&logoColor=white)

**前端**

![Vue.js](https://img.shields.io/badge/Vue.js-4FC08D?style=flat-square&logo=vue.js&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white)
![Pinia](https://img.shields.io/badge/Pinia-FFD859?style=flat-square&logo=vue.js&logoColor=111111)

**工具**

![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat-square&logo=linux&logoColor=111111)

## GitHub 数据

<div align="center">
  <img height="165" src="https://github-readme-stats.vercel.app/api?username=yanhuo218&show_icons=true&hide_border=true&bg_color=00000000&title_color=38BDF8&icon_color=38BDF8&text_color=8B949E&locale=cn" alt="Yanhuo 的 GitHub 统计" />
  <img height="165" src="https://github-readme-streak-stats.herokuapp.com/?user=yanhuo218&hide_border=true&background=00000000&ring=38BDF8&fire=38BDF8&currStreakLabel=38BDF8&sideLabels=8B949E&dates=8B949E" alt="Yanhuo 的连续贡献统计" />
</div>

## 联系我

[![Gmail](https://img.shields.io/badge/Gmail-联系我-EA4335?style=flat-square&logo=gmail&logoColor=white)](mailto:gilbertgtzv66@gmail.com)

<div align="center">

![访问计数](https://komarev.com/ghpvc/?username=yanhuo218&color=38BDF8&style=flat-square&label=Profile+Views)

</div>
